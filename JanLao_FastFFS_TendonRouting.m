...
while inHull == 1;
    if radiusbounds(2) - radiusbounds(1) < tolerance
        ...
        % Create Delaunay triangulation function:
        DT = delaunayTriangulation(W_T(hull,1),W_T(hull,2));
        % DT.Points = vertices
        % DT.ConnectivityList = points of triangle
        % DT.Constraints = constraint list
        
        t_or_f = isa(hul,'triangulation');
        if t_or_f == 0 % If hul is not a triangulation
            DT = delaunayTriangulation(hul(hull,1),hul(hull,2));
            % Then just set it
        end
        
        % Examining all the vertices of DT(W_T) if they are nonzero(1=true)
        was_too_small = all(DT.Points);
